...
public class DictionariesController extends BaseController {
	...
	public ModelAndView save(PrintWriter out) throws Exception{
		...
		if(null == pd.getString("ZD_ID") || "".equals(pd.getString("ZD_ID"))){
			...
			String ZID = String.valueOf(Integer.parseInt(dictionariesService.findMaxId(pd).get("ZID").toString())+1);
			pd.put("ZD_ID", ZID);
			
			//pd.put("ZD_ID", this.get32UUID());	//ID  之前的写法，保存的为32位UUID，改为自增长ID
